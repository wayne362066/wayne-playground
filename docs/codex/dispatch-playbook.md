...
4. 整合結果、檢查修改邊界、執行最低驗證，最後決定完成／阻塞／詢問。

agent 只負責派工訊息內的明確切片。它不得擴大寫入範圍、安裝插件、改變驗收標準、執行未授權破壞性操作或把未跑的驗證寫成通過。
...
| 資訊不足 | 目標檔案／需求／測試 oracle 缺失，兩種方案會造成不同外部行為 | 先讀可取得資料；仍無法安全選擇時向使用者詢問，附上需要的選項與影響 | 使用者選擇或足夠證據出現前不改高風險內容 |
