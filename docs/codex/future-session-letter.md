...
2. `make test` 與 host `cd backend && php artisan test` 都必須強制使用 SQLite `:memory:`，並由 `backend/tests/TestCase.php` 在 migration 前驗證實際設定；它們都不代表 PostgreSQL／Redis 整合通過。Docker socket 或 frontend dependencies 的可用狀態要在每個新 session 重新確認，不能沿用舊結果。
...
- 過往 session 的 host backend、Compose 與 frontend build 結果只屬於當時證據；新 session 必須重跑。任一 PHPUnit 入口只驗證隔離的 SQLite／array 測試後端，不得寫成 PostgreSQL／Redis 整合通過。
...
- 是否另補跨 backend／frontend 的統一 verification target、是否將 frontend dependency install 納入可重複流程：需要使用者授權／產品範圍決策；本輪只修正既有 `make test` 的資料隔離與一次性容器行為，未新增跨層 target 或修改 lockfile。
